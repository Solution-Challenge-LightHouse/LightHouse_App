...
class Middle3_5 extends StatefulWidget {
  const Middle3_5({super.key});

  @override
  State<Middle3_5> createState() => _TodoListPageState();
}

class _TodoListPageState extends State<Middle3_5> {
  bool isLoding = true;
  List items = [];
  String id = '';
  @override
  ...
  Future<void> fetchTodo() async {
    ...
    const url = 'http://52.79.242.2:8080/examples/find?grade=3&category=원의 성질';
    ...
    if (response.statusCode == 200) {
      String? accesstoken = await getToken();
      const url =
          'http://52.79.242.2:8080/examples/find?grade=3&category=원의 성질';
      final uri = Uri.parse(url);
      final response = await http.get(uri, headers: {
        'Content-Type': 'application/json; charset=utf-8',
        'Authorization': 'Bearer $accesstoken'
